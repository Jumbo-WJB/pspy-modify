...
在Linux提权过程中，我们可以尝试高权限运行的文件我们当前用户是否可写，如果可写的话就可以进行提权，详情可见：

https://mp.weixin.qq.com/s/egsHOPK_S5vZujqIb3ygOQ

因此对pspy进行了如下内容的更改：

1. 添加了对进程中的文件参数进行了是否具备写权限判断
2. 添加了对进程打开的fd文件句柄进行了写权限判断

运行程序开始持续监听：

```./go_build_github_com_dominicbreuker_pspy_linux -p -i 1000```

![image-20220119150653534](README.assets/image-20220119150653534.png)

root权限运行一个其他用户可读写的文件：

![image-20220119150752371](README.assets/image-20220119150752371.png)

pspy会在命令行展示结果：

![image-20220119150822398](README.assets/image-20220119150822398.png)

也会在`/tmp/pspyokfile.txt`保存结果：

![image-20220119150859185](README.assets/image-20220119150859185.png)
